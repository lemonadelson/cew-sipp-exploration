...
pu <- fread(ds, sep = "|", select = c(
  ...
  'TPTOTINC','EDEBT_ED','EJSEDDEBT','EOEDDEBT','TJSEDDEBTVAL','TOEDDEBTVAL','TDEBT_ED','TPEARN','RMESR',
...
ds21 <- c("/Users/mca141/Documents/Lumina/lumina-sipp/data/pu2021.csv")
pu21 <- fread(ds21, sep = "|", select = c(
...
# Create data frame of full-year earnings
...
annual_earnings <- pu |>
  ...
  group_by(id) |>
  summarize(year_earn = sum(TPEARN)) |>
  filter(year_earn > 0)
# different months have different weights, but doesn't seem to work to use survey_total...will just rely on weights from Dec
...
```{r}
sippdec_ae <- sippdec |>
  left_join(annual_earnings, by = "id")
```

Survey object
```{r}

sippdecsvy_ae <- sippdec_ae |> as_survey_design(weights = WPFINWGT)
...
## Median education loans and earnings by highest credential and age
...
sippdecsvy_ae |>
  ...
  summarize(
    ...
    medearn_year = survey_median(year_earn),
    unw_count = n()) |>
  print (n = 22)
...
## Identifying possible parents holding ed debt for children
...
sippdec_1824 <- sippdec |>
  filter(!is.na(EPNPAR1),
         age_grp == "18 to 24") |>
  select(id, SSUID, PNUM, ERESIDENCEID, ERELRPE, EED_SCRNR, TEHC_ST, WPFINWGT, ESEX, TAGE_EHC, TOEDDEBTVAL, TDEBT_ED, EPNPAR1, educ_grp_cert, raceeth)
